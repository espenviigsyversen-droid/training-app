...
- `training-plan-controller.js` bygger en skrivefri current+next-preview med feltvis differanse. Den verner fullførte/historiske økter, krever valg ved kollisjoner og intensjonsoverstyringer og respekterer `scheduleAdjustment` uten å materialisere før preview-porten er godkjent.
- `training-plan-ui.js` eier den lokale, mobil-først firestegsflyten for blokkutkast, faktisk volumvalidering, full fireukersoversikt og eksplisitte konfliktvalg. Modulen kan bare kalle controllerens skrivefrie preview og lagrer ingenting.
...
- `training-import-controller.js`: implementert i v176b; ren forhåndsvisnings- og commitplan, konfliktfelt, materialisering og vern mot dobbel målkobling.
- `training-import-ui.js`: implementert i v176b; lokal fillesing, forhåndsvisning, eksplisitte handlinger og sluttrapport med injiserte state-/persistence-avhengigheter.
